...
3. **用 Read 工具打開截圖，肉眼檢查**：版面沒破、視覺層次如預期（標題/表格/卡片的階層一眼可辨）、CI 顏色正確、表格沒有溢出或擠壓。同時對著截圖回答 `../shared/html-craft.md` 第 9 節的四題 craft 自檢（視覺決定來自內容哪裡／密度對比在哪／accent 出現在哪幾處／120% 細節是哪個）——答不出來就回去改，不是照樣交付。發現問題就修，修完重跑驗證。
...
這是最後一段，產出後直接問：「Report 完成了。這套 BRD/PRD/report 需要針對哪個部分再調整嗎，還是先這樣？另外需不需要 PDF 版（傳閱/列印用）？」
