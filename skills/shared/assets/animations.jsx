/**
 * Source: huashu-design (https://github.com/alchaincyf/huashu-design) by alchaincyf, MIT License.
 * Vendored into pm-almighty-box with one patch (pm-demo 動畫時間軸引擎)：
 * 錄製模式下 updateScale 不為播放器條保留 56px，消除導出影片的 letterbox 黑邊。
  ...
  function Stage({ duration = 10, width = 1920, height = 1080, fps = 60, loop = true, children, bgColor = '#fff' }) {
    ...
    useEffect(() => {
      function updateScale() {
        ...
        // pm-almighty-box patch: 錄製/逐幀模式下播放器條被隱藏，不需為它保留 56px——
        // 否則導出的影片四周會有 ~5% letterbox 黑邊（瀏覽器手動觀看時維持原行為）。
        const chromeReserve = (window.__recording || window.__seekRender) ? 0 : 56;
        const vh = window.innerHeight - chromeReserve;
